...
/**
 * App.tsx（manifest不要・命名規則だけで差し替え）
 * - URL: ?ip=<IP名>&cara=<キャラID>
 * - 画像/音声を以下の”規則”で探し、存在すれば採用。無ければ内蔵にフォールバック
...
export default function App() {
  ...
  // フレーム候補（プログラム描画 3種）
  const frameOptions = PROGRAM_FRAMES;
  const [activeFrame, setActiveFrame] = useState<FrameKind>(frameOptions[0].id);
  ...
  const [scale, setScale] = useState(1), [rot, setRot] = useState(0);
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (ps.length===2 && gestureStart.current){
      ...
      let delta=a-g0.a; delta=((delta+180)%360)-180; setRot(g0.rot+delta);
    ...
    } else if (ps.length===1 && !isGesturing){
      const p=ps[0]; const prev=pointers.current.get(e.pointerId)!;
      setCx(v=>v+(p.x-prev.x)); setCy(v=>v+(p.y-prev.y));
    }
  };
  ...
  const doCapture = async () => {
    ...
    if (!usingPlaceholder && videoRef.current && (videoRef.current as any).videoWidth){
      const vw=(videoRef.current as any).videoWidth; const vh=(videoRef.current as any).videoHeight;
    ...
    if (charUrl && stageRef.current){
      ...
      const baseW=Math.min(stageW*baseWidthRatio, 380); const drawW=baseW*scale*ratio;
    ...
    // フレーム合成（規則：/packs/<ip>/frames/<frameId>_<aspect>.(png|webp) を探す）
    const overlay = await pickExisting([
      `/packs/${ip}/frames/${activeFrame}_${aspect.replace(":","x")}.png`,
      `/packs/${ip}/frames/${activeFrame}_${aspect.replace(":","x")}.webp`,
    ], "image");
    if (overlay){
      const img = await loadImage(overlay);
      ctx.drawImage(img, 0, 0, w, h);
    } else {
      // 内蔵PNGが同名であれば使用、なければプログラム描画
      const builtin = Object.entries(builtinFramePNGs).find(([p]) => fileBase(p) === `${activeFrame}_${aspect.replace(":","x")}`)?.[1];
      if (builtin){
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-slate-900 via-slate-900 to-slate-800 text-white p-4 sm:p-8">
      <div className="mx-auto max-w-7xl space-y-4">
        <motion.div className="bg-slate-800/60 rounded-2xl p-5 sm:p-6 shadow-xl border border-white/10" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}>
          ...
          <p className="text-slate-300 mb-4">入口は固定URL。?ip と ?cara のみで素材を切り替え（manifest不要）。<br/>例：<span className="font-mono">?ip=anime1&cara=alice</span></p>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-3">
            ...
            <button onClick={()=>setFacing(p=>p==="user"?"environment":"user")} className="rounded-2xl px-3 py-2 bg-slate-700 hover:bg-slate-600">カメラ切替（今：{facing==="user"?"自撮り":"背面"}）</button>
            ...
            <button onClick={()=>setShowGuide(v=>!v)} className="rounded-2xl px-3 py-2 bg-slate-700 hover:bg-slate-600">ガイド{showGuide?"ON":"OFF"}</button>
          </div>
          ...
          <div ref={stageRef} className="relative w-full overflow-hidden rounded-3xl bg-black select-none" style={{ aspectRatio: aspect.replace(":", "/") }}>
            ...
            {charUrl && (
              <img
                ...
                style={{ touchAction:"none", userSelect:"none", width:`min(${baseWidthRatio*100}%, 380px)`, transform:`translate(calc(-50% + ${cx}px), calc(-50% + ${cy}px)) rotate(${rot}deg) scale(${scale})` }}
                draggable={false}
              />
            )}
  ...
  if (active === "neon") {
    return (
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-xl font-bold text-white" style={{ background: "linear-gradient(90deg, rgba(0,255,255,0.5), rgba(255,0,255,0.5))", textShadow: "0 2px 8px rgba(0,0,0,0.6)" }}>Oshi Camera</div>
      </div>
    );
  }
  return null;
...
// pointer handlers reuse
function onPointerDown(this: any, e: React.PointerEvent){}
function onPointerMove(this: any, e: React.PointerEvent){}
function onPointerUp(this: any, e: React.PointerEvent){}

// NOTE: 上の onPointer* は JSX から参照されるため定義は必要。実体はコンポーネント内にあります（TS型の都合）。
